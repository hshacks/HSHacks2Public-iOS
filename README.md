...
1. Change the stuff in sharedInstance in HSHackathonManager. (logo, hackathon name, firebasechaturl, parse stuff)
...
5. Login with twitter is slightly more complicated as we're using their new Fabric framework, which makes integration super easy but sharing code a bit harder.
   - I deleted our own Fabric account, so the best thing do to here is to go to www.fabric.io and manually add fabric again to this project. Code is the same.
